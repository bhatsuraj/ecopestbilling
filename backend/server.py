...
# ---------------------------------------------------------------------------
# Email (SMTP) — used by the invoice "Share → Email" flow so the customer
# receives the PDF as a real attachment, not via a Gmail compose URL.
# Auth is via a Gmail App Password stored in backend/.env.
# ---------------------------------------------------------------------------
...
async def send_bill_email(payload: SendBillEmailBody):
    ...
    descriptive error if the email could not be delivered (caller surfaces it
    in the share modal).
    """
    ...
    if not smtp_user or not smtp_pass:
        raise HTTPException(
            status_code=500,
            detail="Email not configured. Missing SMTP_USER / SMTP_PASSWORD on the server.",
        )
    ...
    if b64.startswith("data:"):
        ...
        if comma != -1:
            b64 = b64[comma + 1:]
    # Strip whitespace/newlines defensively (base64 from FileReader is clean,
    # but proxies sometimes insert them).
    b64 = re.sub(r"\s+", "", b64)
    try:
    ...
    # Sanitize filename — Gmail accepts most chars but strip CR/LF to avoid
    # header injection.
    ...
    msg["Subject"] = payload.subject or "Invoice"
    msg.set_content(payload.body or "Please find the invoice attached.")
    msg.add_attachment(
        pdf_bytes,
    ...
    # Gmail SMTP: 465 = implicit SSL, 587 = STARTTLS. Branch on port.
    try:
        if smtp_port == 465:
        ...
        raise HTTPException(
            status_code=500,
            detail="Gmail rejected the credentials. Check SMTP_USER / SMTP_PASSWORD and that the App Password is still valid.",
        )
    except (smtplib.SMTPException, OSError) as e:
        logging.error(f"SMTP send failed: {e}")
        raise HTTPException(status_code=500, detail=f"Could not send email: {e}")

    logging.info(f"Email: invoice sent to {payload.to} ({safe_name}, {len(pdf_bytes)} bytes)")
    return {"ok": True, "to": str(payload.to), "filename": safe_name, "size": len(pdf_bytes)}
